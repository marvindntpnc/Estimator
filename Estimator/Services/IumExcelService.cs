...
public class IumExcelService: IExcelService
{
    ...
    public IumExcelService(IWebHostEnvironment env,
    ...
    public async Task<XLWorkbook> FormEstimateExcelBookAsync(Estimate estimate)
    {
        ...
        var estimateContract = await _facilityService.GetContractByIdAsync(estimate.ContractId);
        string estimateContractName =
            $"№{estimateContract.Number} от {estimateContract.StartDate:dd.MM.yyyy}";
        ...
        var ktoMaterials = estimateItems.Where(ei =>
            ei.TarifficatorItem.TarifficatorType == TarifficatorType.KTO &&
            ei.TarifficatorItem.TarificatorItemType == TarificatorItemType.Material).ToList();
        
        var ktoServices = estimateItems.Where(ei =>
            ei.TarifficatorItem.TarifficatorType == TarifficatorType.KTO &&
            ei.TarifficatorItem.TarificatorItemType == TarificatorItemType.Service).ToList();

        string tariffType = ktoMaterials.Any() || ktoServices.Any()? "КТО"
            : fulMaterials.Any() || fulServices.Any() ? "ФУЛ"
            : String.Empty;
        
        FormDocumentHeader(worksheet, estimate, tariffType,estimateContractName);
        ...
        if (ktoMaterials.Any() || ktoServices.Any())
        {
            if (ktoMaterials.Any())
            {
                ...
                worksheet.Cell($"B{rowNumber}").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetOutsideBorder(XLBorderStyleValues.Medium);
                var groupedKtoMaterials = ktoMaterials.GroupBy(fm => fm.TarifficatorItem.CurrencyType).ToList();
                
                foreach (var group in groupedKtoMaterials)
                {
                    var currencyName=EnumHelper.ConvertCurrencyTypeToString(group.Key);
                    if (group.Key!=CurrencyType.RUB)
                    {
                        worksheet.Cell($"D{rowNumber}").Value = $"Курс {currencyName}:";
                        worksheet.Range($"D{rowNumber}", $"E{rowNumber}").Merge();
                        
                        var rate = estimate.CurrencyRate != null && estimate.CurrencyRate.TryGetValue(group.Key, out var r)
                            ? r
                            : 0m;
                        worksheet.Cell($"F{rowNumber}").Value = rate;
                        worksheet.Cell($"F{rowNumber}").Style.NumberFormat.Format = "0.00000";
                    }
                    else
                    {
                        worksheet.Cell($"D{rowNumber}").Value = $"{currencyName}:";
                        worksheet.Range($"D{rowNumber}", $"F{rowNumber}").Merge();
                    }
                    
                    worksheet.Row(rowNumber).Height = 25.5;
                    worksheet.Range($"B{rowNumber}", $"C{rowNumber}").Merge();
                    worksheet.Range($"D{rowNumber}", $"E{rowNumber}").Style.Alignment.WrapText = true;
                    worksheet.Range($"D{rowNumber}", $"E{rowNumber}").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
                    worksheet.Range($"D{rowNumber}", $"E{rowNumber}").Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
                    ...
                    foreach (var ktoMaterialsItem in group) 
                    {
                        await FillItemRow(worksheet, ktoMaterialsItem, estimate, rowNumber);
                        rowNumber++;
                    }
                }
            }
            ...
            if (ktoServices.Any())
            {
                ...
                var groupedKtoServices = ktoServices.GroupBy(fm => fm.TarifficatorItem.CurrencyType).ToList();
                
                foreach (var group in groupedKtoServices)
                {
                    var currencyName=EnumHelper.ConvertCurrencyTypeToString(group.Key);
                    if (group.Key!=CurrencyType.RUB)
                    {
                    ...
                    foreach (var ktoServiceItem in group) 
                    {
                        await FillItemRow(worksheet, ktoServiceItem, estimate, rowNumber);
                        rowNumber++;
                    }
                }
            }
        ...
        if (fulMaterials.Any() || fulServices.Any())
        {
            if (ktoMaterials.Any() || ktoServices.Any())
            {
                worksheet.Cell($"B{rowNumber}").Value =
                    $"\u2116 в тарификаторе ФУЛ к договору {estimateContractName}";
                ...
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetInsideBorder(XLBorderStyleValues.Medium);
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetOutsideBorder(XLBorderStyleValues.Medium);
            ...
            if (fulMaterials.Any())
            {
                var groupedFulMaterials = fulMaterials.GroupBy(fm => fm.TarifficatorItem.CurrencyType).ToList();
                
                foreach (var group in groupedFulMaterials)
                {
                    var currencyName=EnumHelper.ConvertCurrencyTypeToString(group.Key);
                    if (group.Key!=CurrencyType.RUB)
                    {
                    ...
                    foreach (var fulMaterialsItem in group) 
                    {
                        await FillItemRow(worksheet, fulMaterialsItem, estimate, rowNumber);
                        rowNumber++;
                    }
                }
            }
            ...
            if (fulServices.Any())
            {
                ...
                {
                    await FillItemRow(worksheet, fulServicesItem, estimate, rowNumber);
                    rowNumber++;
                }
            }
        }

        worksheet.Range($"B{rowNumber - 1}", $"G{rowNumber - 1}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);

        decimal grandTotal = await _estimateService.CalculateEstimateGrandTotal(estimate.Id);

        worksheet.Cell($"B{rowNumber}").Value = "ИТОГО за услуги без НДС:";
        worksheet.Row(rowNumber).Height = 25.5;
        worksheet.Range($"B{rowNumber}", $"F{rowNumber}").Merge();
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Alignment.Horizontal =
            XLAlignmentHorizontalValues.Right;
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Font.Bold = true;
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
        worksheet.Cell($"G{rowNumber}").Value = await CalculateItemsTotalInRub(estimateItems.Where(ei=>ei.TarifficatorItem.TarificatorItemType==TarificatorItemType.Service).ToList());
        ...
        if (fulMaterials.Any())
        {
            ...
            worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
            worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
            worksheet.Cell($"G{rowNumber}").Value = await CalculateItemsTotalInRub(fulMaterials);
            worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
            rowNumber++;
        }
        ...
        if (ktoMaterials.Any())
        {
            ...
            worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
            worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
            worksheet.Cell($"G{rowNumber}").Value = await CalculateItemsTotalInRub(ktoMaterials);
            worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
            rowNumber++;
        }
        ...
        {
            var materialsList = estimateItems.Where(ei =>
                ei.TarifficatorItem.TarificatorItemType == TarificatorItemType.Material).ToList();
            
            if (estimate.DiscountMaterials == TarifficatorType.FUL)
                materialsList=materialsList.Where(m => m.TarifficatorItem.TarifficatorType == TarifficatorType.FUL).ToList();
            
            if (estimate.DiscountMaterials == TarifficatorType.KTO)
                materialsList=materialsList.Where(m => m.TarifficatorItem.TarifficatorType == TarifficatorType.KTO).ToList();
            
            decimal materialsTotal = await CalculateItemsTotalInRub(materialsList);
            var discountRates=await _estimateService.GetEstimateDiscountListByEstimateIdAsync(estimate,await CalculateItemsTotalInRub(estimateItems));
            if (discountRates != null)
            {
                ...
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetOutsideBorder(XLBorderStyleValues.Medium);
                worksheet.Cell($"F{rowNumber}").Value = Math.Round(discountRates.UninstallRate,2);
                worksheet.Cell($"F{rowNumber}").Style.Alignment.Horizontal =
                    XLAlignmentHorizontalValues.Center;
                worksheet.Cell($"F{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                worksheet.Cell($"G{rowNumber}").Value = CalculateDiscountTotal(materialsTotal,discountRates.UninstallRate);
                worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                rowNumber++;
                ...
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetOutsideBorder(XLBorderStyleValues.Medium);
                worksheet.Cell($"F{rowNumber}").Value = Math.Round(discountRates.InstallRate,2);
                worksheet.Cell($"F{rowNumber}").Style.Alignment.Horizontal =
                    XLAlignmentHorizontalValues.Center;
                worksheet.Cell($"F{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                worksheet.Cell($"G{rowNumber}").Value = CalculateDiscountTotal(materialsTotal,discountRates.InstallRate);
                worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                rowNumber++;
                ...
                worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border
                    .SetOutsideBorder(XLBorderStyleValues.Medium);
                worksheet.Cell($"F{rowNumber}").Value = Math.Round(discountRates.SuppliesRate,2);
                worksheet.Cell($"F{rowNumber}").Style.Alignment.Horizontal =
                    XLAlignmentHorizontalValues.Center;
                worksheet.Cell($"F{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                worksheet.Cell($"G{rowNumber}").Value = CalculateDiscountTotal(materialsTotal,discountRates.SuppliesRate);
                worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
                rowNumber++;
            }
        }
        ...
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
        worksheet.Cell($"G{rowNumber}").Value =Math.Round(grandTotal*100/120,2);
        worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
        rowNumber++;
        ...
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
        worksheet.Cell($"G{rowNumber}").Value = Math.Round(grandTotal*20/120,2);
        worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
        rowNumber++;
        ...
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetInsideBorder(XLBorderStyleValues.Medium);
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
        worksheet.Cell($"G{rowNumber}").Value = Math.Round(grandTotal,2);
        worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
        worksheet.Range("B4", $"G{rowNumber}").Style.Border.SetOutsideBorder(XLBorderStyleValues.Medium);
        
        rowNumber += 2;

        worksheet.Cell($"B{rowNumber}").Value =
            "* срок поставки и выполнения работ с момента согласования - 4-8 раб. недели";
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Merge();
        rowNumber += 3;

        worksheet.Cell($"B{rowNumber}").Value =
            "* Подписание сметного расчета, означает согласование Заказчика с собственником проведение всех работ,  замены материалов и запчастей";
        worksheet.Range($"B{rowNumber}", $"G{rowNumber}").Merge();
        rowNumber += 2;
        ...
        worksheet.Range($"F{rowNumber}", $"G{rowNumber}").Style.Alignment.Horizontal =
            XLAlignmentHorizontalValues.Right;
        rowNumber++;

        worksheet.Cell($"B{rowNumber}").Value = "ООО \"Инженер Юг Монтаж\"";
        worksheet.Range($"B{rowNumber}", $"C{rowNumber}").Merge();
        worksheet.Range($"B{rowNumber}", $"C{rowNumber}").Style.Font.Bold = true;
        ...
        worksheet.Range($"D{rowNumber}", $"G{rowNumber}").Style.Alignment.Horizontal =
            XLAlignmentHorizontalValues.Right;
        rowNumber++;

        worksheet.Cell($"B{rowNumber}").Value = "___________________/Петряев С.В./";
        worksheet.Range($"B{rowNumber}", $"C{rowNumber}").Merge();
        ...
        worksheet.Range($"E{rowNumber}", $"G{rowNumber}").Style.Alignment.Horizontal =
            XLAlignmentHorizontalValues.Right;
    ...
    private async Task FillItemRow(IXLWorksheet worksheet, EstimateItemModel item, Estimate estimate,
        int rowNumber)
    {
        var facility=await _facilityService.GetFacilityByIdAsync(estimate.FacilityId);
        worksheet.Cell($"B{rowNumber}").Value = $"{item.TarifficatorItem.ItemCode}";
        worksheet.Cell($"C{rowNumber}").Value = $"{item.TarifficatorItem.Name}";
        worksheet.Cell($"C{rowNumber}").Style.Alignment.WrapText = true;
        worksheet.Cell($"D{rowNumber}").Value = item.TarifficatorItem.Measure==MeasureType.Hours?
            item.CustomRate>0? 
                item.Qty*item.TarifficatorItem.Price*item.CustomRate: 
                item.Qty*item.TarifficatorItem.Price:
            item.Qty;
        worksheet.Cell($"E{rowNumber}").Value =
            EnumHelper.ConvertMeasureTypeToString(item.TarifficatorItem.Measure);
        worksheet.Cell($"F{rowNumber}").Value = item.TarifficatorItem.Measure==MeasureType.Hours?facility.HourRate :item.TarifficatorItem.Price;//await _estimateService.CalculateEstimateItemRublePrice(item);
        worksheet.Range($"D{rowNumber}",$"F{rowNumber}").Style.Alignment.Horizontal =
            XLAlignmentHorizontalValues.Center;
        worksheet.Cell($"F{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
        worksheet.Cell($"G{rowNumber}").Value = await _estimateService.CalculateEstimateItemRubleTotal(item);
        worksheet.Cell($"G{rowNumber}").Style.NumberFormat.Format = "# ### ##0.00";
    ...
    private async Task<decimal> CalculateItemsTotalInRub(List<EstimateItemModel> items)
    {
        decimal totalInRub = 0;
        foreach (var item in items)
        {
            totalInRub += await _estimateService.CalculateEstimateItemRubleTotal(item);
        }

        return Math.Round(totalInRub,2);
    }

    private void FormDocumentHeader(IXLWorksheet worksheet,Estimate estimate,string tariffType,string estimateContractName)
    {
        ...
        worksheet.Cell("B2").Value =
            "ООО «Инженер Юг Монтаж», ИНН 6141047203, КПП 614101001\nОГРН 1156181000205, ОКПО 13476603\nЮридический Адрес: 346880, Ростовская область, г. Батайск\nул. Кирова 28 а, офис \u211619\n тел: (863) 206-69-89\nр/сч: 40702810026050000383, ФИЛИАЛ \"РОСТОВСКИЙ\" АО \"АЛЬФА-БАНК\"\nк/сч: 30101810500000000207 БИК 046015207";
        ...
        worksheet.Cell("B3").Value = $"Сметный расчет на  ДР \u2116 {estimate.Number} от {estimate.CreatedOn:dd.MM.yyyy}";
        ...
        worksheet.Range("B3", "G3").Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
         ...
         worksheet.Cell("B4").Value =
            $"\u2116 в тарификаторе {tariffType} к договору {estimateContractName}";
        ...
        worksheet.Cell("C5").Value = estimate.EstimateName;
    ...
    private decimal CalculateDiscountTotal(decimal materialsTotal, decimal discountRate)
    {
        return materialsTotal / 100 * discountRate;
    }
}